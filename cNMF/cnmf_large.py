...
def get_highvar_genes_sparse(expression, expected_fano_threshold=None,
    ...
    # This else statement will never be reached, since numgenes has default value in main
    else:
...
class cNMF():
    ...
    def _initialize_dirs(self):
        if self.paths is None:
            ...
            self.paths = {
                'normalized_counts': os.path.join(self.output_dir, self.name, 'cnmf_tmp',
                                                  self.name + '.norm_counts.h5ad'),
                'nmf_replicate_parameters': os.path.join(self.output_dir, self.name, 'cnmf_tmp',
                                                         self.name + '.nmf_params.df.npz'),
                'nmf_run_parameters': os.path.join(self.output_dir, self.name, 'cnmf_tmp',
                ...
                'iter_spectra': os.path.join(self.output_dir, self.name, 'cnmf_tmp',
                                             self.name + '.spectra.k_%d.iter_%d.df.npz'),
                'iter_usages': os.path.join(self.output_dir, self.name, 'cnmf_tmp',
                                            self.name + '.usages.k_%d.iter_%d.df.npz'),
                'merged_spectra': os.path.join(self.output_dir, self.name, 'cnmf_tmp',
                                               self.name + '.spectra.k_%d.merged.df.npz'),

                'local_density_cache': os.path.join(self.output_dir, self.name, 'cnmf_tmp',
                                                    self.name + '.local_density_cache.k_%d.merged.df.npz'),
                'consensus_spectra': os.path.join(self.output_dir, self.name, 'cnmf_tmp',
                                                  self.name + '.spectra.k_%d.dt_%s.consensus.df.npz'),
                'consensus_spectra__txt': os.path.join(self.output_dir, self.name,
                                                       self.name + '.spectra.k_%d.dt_%s.consensus.txt'),
                'consensus_usages': os.path.join(self.output_dir, self.name, 'cnmf_tmp',
                                                 self.name + '.usages.k_%d.dt_%s.consensus.df.npz'),
                'consensus_usages__txt': os.path.join(self.output_dir, self.name,
                                                      self.name + '.usages.k_%d.dt_%s.consensus.txt'),

                'consensus_stats': os.path.join(self.output_dir, self.name, 'cnmf_tmp',
                ...
                'gene_spectra_score': os.path.join(self.output_dir, self.name, 'cnmf_tmp',
                                                   self.name + '.gene_spectra_score.k_%d.dt_%s.df.npz'),
                'gene_spectra_score__txt': os.path.join(self.output_dir, self.name,
                                                        self.name + '.gene_spectra_score.k_%d.dt_%s.txt'),
                'gene_spectra_tpm': os.path.join(self.output_dir, self.name, 'cnmf_tmp',
                                                 self.name + '.gene_spectra_tpm.k_%d.dt_%s.df.npz'),
                'gene_spectra_tpm__txt': os.path.join(self.output_dir, self.name,
    ...
    def run_nmf(self,
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=total_workers) as executor:
            futures = [executor.submit(self._run_nmf_worker, i,
                                       norm_counts, _nmf_kwargs, run_params) for i in range(len(run_params))]
    ...
    def _run_nmf_worker(self, idx, norm_counts, _nmf_kwargs, run_params):
        p = run_params.iloc[idx, :]
...
def initialize(name, output_dir):
    ...
    Parameters:
        ...
        output_dir: All output will be placed in [output_dir]/[name]/...
...
def prepare(cnmf_obj, counts, components, densify=False, tpm=None, genes_file=None, numgenes=2000, n_iter=100,
            seed=None, beta_loss='frobenius', only_k=False):
    """
    Parameters:

        cnmf_obj: A cNMF object (should already be initialized)
        counts: File name of (cell x gene) matrix with normalized gene expression counts
        components: Number of components (k) for matrix factorization, several should be specified as a list
        densify: Treat the input data as non-sparse
        tpm: Pre-computed (cell x gene) TPM values as df.npz or tab separated txt file. If not provided, TPM
             will be calculated automatically
        genes_file: File containing a list of genes to include, one gene per line. Must match column labels of
                    counts matrix
        num_genes: Number of high variance genes to use for matrix factorization
        n_iter: Numper of factorization replicates
        seed: Seed for pseudorandom number generation
        beta_loss: Loss function for NMF
        only_k: Only change nmf_params.df.npz to have values specified by 'components'

    """

    if not only_k:
        if counts.endswith('.h5ad'):
            input_counts = sc.read(counts)
        else:
            # Load txt or compressed dataframe and convert to scanpy object
            if counts.endswith('.npz'):
                input_counts = load_df_from_npz(counts)
            else:
                input_counts = pd.read_csv(counts, sep='\t', index_col=0)
        ...
        elif tpm.endswith('.h5ad'):
            subprocess.call('cp %s %s' % (tpm, cnmf_obj.paths['tpm']), shell=True)
            tpm = sc.read(cnmf_obj.paths['tpm'])
        else:
            if tpm.endswith('.npz'):
                tpm = load_df_from_npz(tpm)
            else:
        ...
        norm_counts = cnmf_obj.get_norm_counts(input_counts, tpm, num_highvar_genes=numgenes,
...
def factorize(cnmf_obj, total_workers=1):
    ...
    Parameters:
        ...
        cnmf_obj: A cNMF object (should already be initialized)
        total_workers: Total number of workers to distribute jobs to

    """
    cnmf_obj.run_nmf(total_workers=total_workers)


def combine(cnmf_obj, components=None):
    """
    Parameters:

        cnmf_obj: A cNMF object (should already be initialized)
        components: A list of components (k) to be combined (must already created from factorization step)
...
def consensus(cnmf_obj, components=None, local_density_threshold=0.5, local_neighborhood_size=0.30,
              show_clustering=True):
    """
    Parameters:

        cnmf_obj: A cNMF object (should already be initialized)
        components: A list of components (k) to conduct consensus with
        local_density_threshold: Threshold for the local density filtering. This is a float >0 and <=2
        local_neighborhood_size: Fraction of the number of replicates to use as nearest neighbors for local density filtering
        show_clustering: Produce a clustergram figure summarizing the spectra clustering
    ...
    for k in ks:
        ...
        cnmf_obj.consensus(k, local_density_threshold, local_neighborhood_size, show_clustering)


def k_selection_plot(cnmf_obj):
    """
    Parameters:
...
def get_usage_df(cnmf_obj, selected_k, density_threshold, normalized=True):
    density_threshold_str = str(density_threshold).replace('.', '_')
    usage = pd.read_csv(cnmf_obj.paths['consensus_usages__txt'] % (selected_k, density_threshold_str),
                        sep='\t', index_col=0)
    usage.columns = ['Usage_{}'.format(i) for i in usage.columns]
    if normalized:
...
def get_gene_scores(cnmf_obj, selected_k, density_threshold):
    density_threshold_str = str(density_threshold).replace('.', '_')

    gene_scores = pd.read_csv(cnmf_obj.paths['gene_spectra_score__txt'] % (selected_k, density_threshold_str),
                              sep='\t', index_col=0).T
    return gene_scores


def get_top_genes(cnmf_obj, selected_k, density_threshold, ngenes=50):
    gene_scores = get_gene_scores(cnmf_obj, selected_k, density_threshold)
    top_genes = []
    ...
    top_genes.columns = [f'Usage_{i}' for i in top_genes.columns]
    return top_genes
...
if __name__ == "__main__":
    ...
    parser.add_argument('--name', type=str,
                        help='[all] Name for analysis. All output will be placed in [output-dir]/[name]/...', nargs='?',
                        default='cNMF')
    parser.add_argument('--output-dir', type=str,
                        help='[all] Output directory. All output will be placed in [output-dir]/[name]/...', nargs='?',
                        default='.')
    ...
    parser.add_argument('--total-workers', type=int, help='[all] Total number of workers to distribute jobs to',
                        default=1)
    ...
    parser.add_argument('--tpm', type=str,
                        ...
                        default=None)
    parser.add_argument('--beta-loss', type=str, choices=['frobenius', 'kullback-leibler', 'itakura-saito'],
                        help='[prepare] Loss function for NMF.', default='frobenius')
    parser.add_argument('--densify', dest='densify', help='[prepare] Treat the input data as non-sparse',
                        action='store_true', default=False)

    parser.add_argument('--worker-index', type=int,
                        help='[factorize] Index of current worker (the first worker should have index 0)', default=0)

    parser.add_argument('--local-density-threshold', type=str,
                        help='[consensus] Threshold for the local density filtering. This is a float >0 and <=2',
                        default=0.5)
    ...
    cnmf_obj = initialize(args.name, args.output_dir)

    if args.command == 'prepare':

        prepare(cnmf_obj, args.counts, args.components, args.densify, args.tpm, args.genes_file, args.numgenes,
                args.n_iter, args.seed, args.beta_loss)


    elif args.command == 'factorize':

        factorize(cnmf_obj, args.total_workers)

    elif args.command == 'combine':

        combine(cnmf_obj, args.components)

    elif args.command == 'consensus':

        consensus(cnmf_obj, args.components, args.local_density_threshold, args.local_neighborhood_size,
                  args.show_clustering)

    elif args.command == 'k_selection_plot':

        k_selection_plot(cnmf_obj)
